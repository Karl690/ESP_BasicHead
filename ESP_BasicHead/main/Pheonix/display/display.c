...
void WriteGuiBuffer()
{
	FillRGBRect(0, 0, LCD_WIDTH, LCD_HEIGHT, GUI_BUF);
}
void Format_Int32(uint8_t row, void* info, uint16_t* colorTable)
{
	varInfo = (LcdVariableInfo*)info;
	DrawString(LEFT_PADDING, PADDING + row,varInfo->Label, colorTable[varInfo->Color_1]);
	sprintf(strTempVal, "%d ", (int)(*((uint32_t*)varInfo->VariablePointer)));
	DrawString(VALUE_POS, PADDING + row, strTempVal, colorTable[varInfo->Color_2]);
}
...
void Format_Hex8(uint8_t row, void* info, uint16_t* colorTable)
{
	...
	DrawString(LCD_WIDTH/2, PADDING + row, strTempVal, colorTable[varInfo->Color_2]);
...
{
	varInfo = (LcdVariableInfo*)info;
	DrawString(LEFT_PADDING, PADDING + row, varInfo->Label, colorTable[varInfo->Color_1]);//label?
...
void Format_MemoryDisplayAsciiHex(uint8_t row, void* info, uint16_t* colorTable)
{
	...
	varInfo = (LcdVariableInfo*)info;
	DrawString(LEFT_PADDING, PADDING + row, workstring, colorTable[varInfo->Color_2]);

	DrawString(LCD_WIDTH * 0.33f, PADDING + row, strTempVal, colorTable[varInfo->Color_2]);
...
void Format_MemoryDumpHex(uint8_t row, void* info, uint16_t* colorTable)
{
	...
	DrawString(LEFT_PADDING, PADDING + row, strTempVal, colorTable[varInfo->Color_2]);
}
...
void Format_Bar_Dutty(uint8_t row, void* info, uint16_t* colorTable)
{
	...
	if(X2 > X1)	FillRect(X1, PADDING + row, X2, LCD_LINESIZE-PADDING + row, colorTable[varInfo->Color_2]);
	if(X3 > X2) FillRect(X2, PADDING + row, X3, LCD_LINESIZE-PADDING + row, colorTable[COLOR_GRAY]);
...
void Format_Bar_Status(uint8_t row, void* info, uint16_t* colorTable)
{
	...
	if(val) {
		FillRect(X1, PADDING + row, X2, LCD_LINESIZE-PADDING + row, colorTable[varInfo->Color_2]);
	}else {
		FillRect(X1, PADDING + row, X2, LCD_LINESIZE-PADDING + row, colorTable[COLOR_GRAY]);
	}
	sprintf(strTempVal, "%d", val);
	DrawString((X1 + X2)/2, PADDING + row, strTempVal, colorTable[varInfo->Color_2]);
}
...
void Format_Bar_Float(uint8_t row, void* info, uint16_t* colorTable)
{
	...
	if(X2 > X1)	FillRect(X1, PADDING + row, X2, LCD_LINESIZE-PADDING + row, colorTable[varInfo->Color_2]);
	if(X3 > X2) FillRect(X2, PADDING + row, X3, LCD_LINESIZE-PADDING + row, colorTable[COLOR_GRAY]); //BLACK);//

	sprintf(strTempVal, "%.3f", val);
	DrawString((X1 + X3)/2 - 10, PADDING + row, strTempVal, colorTable[varInfo->Color_2]);
}
...
{
	ResetGuiBuffer(BLACK);
	uint16_t* colorTable = ColorSwapTable;
...
void UpdateScreenMemoryHex(uint8_t* label, uint8_t* data, uint16_t size)
{
	uint16_t row = 0, col = 0, cnt = 0;
	ResetGuiBuffer(ColorNormalTable[COLOR_BLACK]);
	DrawString(LEFT_PADDING, PADDING, (char*)label, ColorNormalTable[COLOR_RED]);
	WriteGuiBuffer(row);
	row ++;
...
void UpdateScreenMemoryText(uint8_t* label, uint8_t* data, uint16_t size)
{
	uint16_t row = 0, col = 0, cnt = 0;
	ResetGuiBuffer(ColorNormalTable[COLOR_BLACK]);
	DrawString(LEFT_PADDING, PADDING, (char*)label, ColorNormalTable[COLOR_RED]);
