...
uint16_t* GUI_BUF = NULL;//[LCD_HEIGHT][LCD_HEIGHT];
...
void DrawHLine(uint16_t X1, uint16_t X2, uint16_t Y, uint16_t color)
{
	...
	for(i = X1; i <= X2; i++)
	{
		if( i < LCD_WIDTH) GUI_BUF[Y * LCD_WIDTH + i] = color;
	}
}
void DrawVLine(uint16_t X, uint16_t Y1, uint16_t Y2, uint16_t color)
{
	...
	for(i = Y1; i <= Y2; i++)
	{
		if( i < LCD_HEIGHT) GUI_BUF[i * LCD_WIDTH  + X] = color;
	}
}
	...
	for(t=0;t<GUI_FONT_SIZE;t++)
	{
		...
		for(t1=0;t1<8;t1++)
		{
			if(temp&0x80)
			{
				...
				GUI_BUF[yindex * LCD_WIDTH + xindex] = color;
			}else {
